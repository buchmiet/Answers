...
public partial class UtilityLayerClass:IAnswerable
{
    ...
    {
        using (var response = Answer.Prepare($"GetOrderAndProductsData({orderId})"))
        {
            Answer resp = await TryAsync(() => _serviceTier.GetOrderData(orderId, ct), ct, TimeSpan.FromSeconds(8));
            if (!resp.IsSuccess)
            {
                return resp.Error($"Could not fetch order {orderId}");
            }

            Answer resp2 = await TryAsync(() => _serviceTier.GetProductData(orderId, ct), ct, TimeSpan.FromSeconds(8));
            return !resp2.IsSuccess
                ? resp.Error($"Could not fetch product {orderId}")
                : response.WithValue($"{resp.GetValue<string>()} {resp2.GetValue<string>()}");
        }
    }
}
